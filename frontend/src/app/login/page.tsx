...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .demo-btn:hover { background: var(--bg-2); border-color: var(--border-2); }
  .demo-role {
  ...
  .demo-email {
    font-family: var(--font-mono); font-size: 11px; color: var(--text-muted); flex: 1;
  }
  .demo-arrow { font-size: 12px; color: var(--text-dim); }
...
export default function LoginPage() {
  ...
  const quickLogin = async (e: string, p: string) => {
    setEmail(e); setPassword(p); setError(''); setLoading(true);
    try { await login(e, p); router.push('/dashboard'); }
    catch (err: unknown) { setError(err instanceof Error ? err.message : 'Login failed'); }
    finally { setLoading(false); }
  };
      ...
      <div className="page">
        <div className="card">
          ...
          <form onSubmit={handleSubmit}>
            <div className="field">
              ...
              <input
                ...
                placeholder="you@ethara.ai" required
              />
            </div>
            <div className="field">
            ...
            <button className="btn-submit" type="submit" disabled={loading}>
              {loading
                ? 'Signing in...'
                : <>Sign in <svg width="13" height="13" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path d="M5 12h14M12 5l7 7-7 7"/></svg></>
              }
            </button>
          </form>
          ...
          <div className="demo-list">
            <button className="demo-btn" onClick={() => quickLogin('lead@ethara.ai', 'password123')} disabled={loading}>
              <span className="demo-role admin">Admin</span>
              <span className="demo-email">lead@ethara.ai</span>
              <span className="demo-arrow">→</span>
            </button>
            <button className="demo-btn" onClick={() => quickLogin('aryan@ethara.ai', 'password123')} disabled={loading}>
              <span className="demo-role member">Member</span>
              <span className="demo-email">aryan@ethara.ai</span>
              <span className="demo-arrow">→</span>
            </button>
          </div>
